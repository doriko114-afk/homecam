...
def trigger_special_move(command_char):
    ...
    # 'u' 동작은 약 0.84초 소요, 'n' 동작은 약 0.9초 소요, 'i' 동작은 즉시 실행되지만 안전하게 0.5초 대기
    if command_char == b'u' or command_char == b'n':
        time.sleep(1.0) 
    elif command_char == b'i':
        time.sleep(0.5) # 홈 포지션 이동은 비교적 빠르므로 짧게 대기
...
thumbs_up_cooldown_active = False # '엄지 척' 제스처 쿨다운
thumbs_up_cooldown_start_time = 0
THUMBS_UP_COOLDOWN_PERIOD = 2.0 # '엄지 척' 제스처 감지 후 2초간 쿨다운
...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            # --- '엄지 척' 제스처 판단 ---
            # 엄지는 펴고, 나머지 손가락은 구부림
            thumb_up = is_thumb_straight(landmarks)
            index_curled = not is_finger_straight(landmarks, mp_hands.HandLandmark.INDEX_FINGER_TIP, mp_hands.HandLandmark.INDEX_FINGER_PIP, mp_hands.HandLandmark.INDEX_FINGER_MCP)
            middle_curled = not is_finger_straight(landmarks, mp_hands.HandLandmark.MIDDLE_FINGER_TIP, mp_hands.HandLandmark.MIDDLE_FINGER_PIP, mp_hands.HandLandmark.MIDDLE_FINGER_MCP)
            ring_curled_thumb = not is_finger_straight(landmarks, mp_hands.HandLandmark.RING_FINGER_TIP, mp_hands.HandLandmark.RING_FINGER_PIP, mp_hands.HandLandmark.RING_FINGER_MCP)
            pinky_curled_thumb = not is_finger_straight(landmarks, mp_hands.HandLandmark.PINKY_TIP, mp_hands.HandLandmark.PINKY_PIP, mp_hands.HandLandmark.PINKY_MCP)

            if thumb_up and index_curled and middle_curled and ring_curled_thumb and pinky_curled_thumb:
    ...
    if thumbs_up_detected and not thumbs_up_cooldown_active:
        trigger_special_move(b'i') # 'i' 명령 전송 (홈 포지션)
        thumbs_up_cooldown_active = True
        thumbs_up_cooldown_start_time = time.time()
    ...
    if thumbs_up_cooldown_active and (time.time() - thumbs_up_cooldown_start_time > THUMBS_UP_COOLDOWN_PERIOD):
        thumbs_up_cooldown_active = False
